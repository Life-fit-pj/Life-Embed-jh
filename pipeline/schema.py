...
def add_snapshot_columns(con, table, columns, types):
    ...
    for col in columns:
        ...
        con.execute(text(f'ALTER TABLE "{table}" ADD COLUMN "{snapshot}" {types.get(col, "FLOAT")}'))
...
def enable_fk(con):
    ...
    SQLite 는 이 검사가 기본으로 꺼져 있어 PRAGMA 로 켜야 한다.
    PostgreSQL 은 끌 수가 없으므로 할 일이 없다 — 조용히 지나간다.

    ⚠ PRAGMA 는 연결마다 따로 적용된다. engine 은 연결을 풀에 넣고 돌려쓰므로,
      새 연결을 열 때마다 이걸 불러야 한다
    """
    if is_sqlite():
        con.execute(text("PRAGMA foreign_keys = ON"))


# ① DB 있으면 물어보고 → 지우기 (SQLite 는 파일 삭제, 그 밖은 DROP TABLE)
# ② engine.begin() 으로 트랜잭션 하나 열기. SQLite 면 FK 검사 켜기
# ③ 순서대로 CREATE TABLE
# ④ 데이터 넣기 (INSERT)
# ⑤ FK 칸에 색인 만들기
# ⑥ SQLite 면 FK 위반을 몰아서 검사

if __name__ == "__main__" :
    order = sort_by_dependency(tables)

    # 1. 기존 DB 확인.
    #    "파일을 지운다" 는 SQLite 라서 되는 일이다. Postgres 는 파일이 아니라
    #    서버라서 지울 파일이 없다 — 대신 표를 하나씩 떨어뜨린다
    if is_sqlite():
        if DB_PATH.exists():
        ...
        if answer.lower() != "y" :
            print("중단합니다!")
            sys.exit(0)
        # 만드는 순서의 반대로 지운다 — 참조하는 쪽을 먼저 없애야
        # 참조당하는 쪽을 지울 수 있다
    ...
    # 6. FK 위반 검사.
    #    SQLite 는 PRAGMA foreign_keys 를 켜기 전에 들어간 줄이 있을 수 있어
    #    끝나고 한 번 몰아서 확인해야 한다.
    #    Postgres 는 위반되는 INSERT 자체가 그 자리에서 거부되므로,
    #    여기까지 왔다는 것이 곧 위반이 없다는 뜻이다 — 검사할 것이 없다
    if is_sqlite():
        with engine.connect() as con:
            enable_fk(con)
            for r in con.execute(text("PRAGMA foreign_key_check")).fetchall():
                print(r)
    print("검사 끝")