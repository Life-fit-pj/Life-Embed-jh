...
from app.domain.dong import dong_params
...
def region_one(gu, dong, columns):
    """행정동 하나의 지정한 칸들만 꺼낸다"""
    marks, params = dong_params(gu, dong)
    quoted = ", ".join(f'"{c}"' for c in columns)

    rows = dicts(
        f'SELECT 구, 행정동명, {quoted} FROM master_dataset_v3 '
        f'WHERE TRIM(구) = :gu AND TRIM(행정동명) IN ({marks})',
        params,
    )
...
def facilities(gu, dong, kind=None, limit=10):
    ...
    for label, (table, gu_col, dong_col, name_col, cat_col) in targets.items():
        rows = dicts(
            ...
            f'FROM "{table}" '
            f'WHERE TRIM("{gu_col}") = :gu AND TRIM("{dong_col}") IN ({marks}) '
            f'LIMIT :limit',
            {**params, "limit": limit},
        )
        if rows:
...
def facility_categories(gu, dong, kind="학원", top=8):
    ...
    return dicts(
        ...
        f'GROUP BY "{cat_col}" ORDER BY n DESC LIMIT :top',
        {**params, "top": top},
    )
...
def column_percentile(column, value, invert=False):
    ...
    below = one(
        f'SELECT COUNT(*) FROM master_dataset_v3 WHERE "{column}" < :value',
        {"value": value},
    )[0]
    same = one(
        f'SELECT COUNT(*) FROM master_dataset_v3 WHERE "{column}" = :value',
...
def region_price_detail(gu, dong, bldg, deal):
    ...
    rows = dicts(
        ...
        'FROM 시세_지역별_전처리 '
        f'WHERE TRIM(자치구명) = :gu AND TRIM(지역명) IN ({marks}) '
        '      AND 건물용도 = :bldg AND 거래유형 = :deal',
        {**params, "bldg": bldg, "deal": deal},
    )
    return rows[0] if rows else None
...
def update_region(gu, dong, patch, allowed):
    ...
    marks, params = dong_params(gu, dong)
    where_sql = f'TRIM(구) = :gu AND TRIM(행정동명) IN ({marks})'
    return _run_update("master_dataset_v3", where_sql, params, patch, allowed)
